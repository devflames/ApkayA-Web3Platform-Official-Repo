...
/**
 * Signs and broadcasts a single queued transaction, then waits (in the
 * background, not blocking the worker loop) for it to be mined.
 *
 * Gas strategy: pulls the network's current fee data and applies a 20%
 * buffer on maxPriorityFeePerGas to reduce the chance of being stuck
 * during fee spikes ("gas-optimized retries" from the spec).
 */
async function processTransaction(tx: TransactionRecord): Promise<void> {
  const provider = getProvider(tx.chain_id);

  try {
    const signer = getSignerForWallet(tx.from_wallet_id, tx.chain_id);
    ...
    const [feeData, nonce, gasEstimate] = await Promise.all([
      ...
      provider
        ...
        .catch(() => 100_000n), // fall back to a safe default if estimation fails (e.g. new contract)
    ]);
    ...
    log.info({ txId: tx.id, hash: sent.hash, chainId: tx.chain_id }, "transaction sent");
    await fireWebhook(tx.id, "tx.sent", { hash: sent.hash, chainId: tx.chain_id });

    // Confirmation happens async so the worker can keep draining the queue.
    ...
    if (tx.retry_count < MAX_RETRIES) {
      requeueForRetry(tx.id);
    } else {
      markErrored(tx.id, message, false);
...
async function confirmTransaction(txId: string, hash: string, chainId: number): Promise<void> {
  ...
  if (receipt.status === 1) {
    markMined(txId, receipt.blockNumber);
    ...
    markReverted(txId, receipt.blockNumber);
...
async function loop(): Promise<void> {
  const batch = claimQueuedTransactions(BATCH_SIZE);
  if (batch.length > 0) {
    log.debug({ count: batch.length }, "processing batch");
    // Process in parallel — nonce correctness is per-wallet, and getTransactionCount("pending")
    // at send time keeps same-wallet transactions ordered well enough for v0. For high-throughput
    // single-wallet bursts, switch to a per-wallet in-memory nonce manager.
